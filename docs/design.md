...
| `intonation_score` | integer | not null, default `0`, check between 0 and 50 | 音準歷史最高分 |
| `breath_control_score` | integer | not null, default `0`, check between 0 and 50 | 氣息控制歷史最高分 |
| `rhythm_score` | integer | not null, default `0`, check between 0 and 50 | 節奏歷史最高分 |
| `expression_score` | integer | not null, default `0`, check between 0 and 50 | 情感歷史最高分 |
| `technique_score` | integer | not null, default `0`, check between 0 and 50 | 技巧歷史最高分 |
...
- `index users_intonation_score_idx on users (intonation_score desc)`：音準單項排行使用。
...
- 五項分數 key：`intonation`、`breath_control`、`rhythm`、`expression`、`technique`，每項 `0–50`。
...
Query：
- `type`: required，可為 `total`、`intonation`、`breath_control`、`rhythm`、`expression`、`technique`。
